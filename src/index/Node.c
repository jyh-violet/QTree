...
inline void NodeAddInsertWriteLock(Node* node){
    ...
    vmlog(RemoveLog,"NodeAddInsertWriteLock node:%d, %x", ((Node*)node)->id, node->insertLock);
...
void NodeDegradeInsertLock(Node* node, int threadId){
    ...
    LockType  flag = node->insertLock;
    LockType target =  (1L << (threadId + RemoveReadOff)) | (flag & 2L);
...
BOOL NodeTryAddInsertReadLock(Node* node, int threadId){
    vmlog(RemoveLog,"NodeTryAddInsertReadLock node:%d, %x", ((Node*)node)->id, node->insertLock);
    //    int tid = getThreadId();
    int try = 0;
    while (try < TryCount){
        LockType  flag = node->insertLock;
...
void NodeRmInsertReadLock(Node* node, int threadId){
    //    int tid = getThreadId();
    vmlog(InsertLog,"NodeRmInsertReadLock node:%d, %x", ((Node*)node)->id, node->insertLock);
...
//    vmlog(RemoveLog,"NodeAddRemoveWriteLockNoWait node:%d ", ((Node*)node)->id);
    while (1){
        if(__sync_bool_compare_and_swap(&node->removeLock, 0L, 3L)){
...
void NodeRmRemoveWriteLock(Node* node){
    __sync_bool_compare_and_swap(&node->removeLock, 3L, 0L);
